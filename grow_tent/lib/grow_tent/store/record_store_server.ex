defmodule GrowTent.Record.StoreServer do
  use GenServer

  require Logger

  alias GrowTent.Record.Store

  # Client
  def start_link(params) when is_list(params) do
    config_store_file =
      Application.get_env(:ui, GrowTent.Record)[:config_store_versioned_file_path] || "tmp/record.db"
  ...
  def handle_continue(:create_table, %{file: file, table: table} = state) do
    _ = Logger.debug("Record Store File: #{inspect(file)}")

    if File.exists?(file) do
      {:ok, _ref} = Store.load_table_from_file(table, file)
    else
      _ = Logger.warn("No store file: #{inspect(file)}, located by RecordStoreServer")
